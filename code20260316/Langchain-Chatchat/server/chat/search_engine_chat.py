...
def custom_search(text, result_len=SEARCH_ENGINE_TOP_K, **kwargs):
    from server.web_utils import down_web_page_content_with_query
    import re

    docs = down_web_page_content_with_query(text, result_len)
    return [{"snippet": re.sub(r'[^\u4e00-\u9fa5\s]', '', x.page_content), "link": "http://unknown", "title": text} for x in docs]
