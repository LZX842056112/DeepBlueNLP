...
from configs import LOG_PATH, TEMPERATURE, logger
...
class ApiModelWorker(BaseModelWorker):
    ...
    def generate_stream_gate(self, params: Dict):
        self.call_ct += 1
        logger.info(f"在线模型worker进程中执行的方法 generate_stream_gate :{params}")
    ...
    def generate_gate(self, params):
        try:
            logger.info(f"在线模型worker进程中执行的方法 generate_gate :{params}")
