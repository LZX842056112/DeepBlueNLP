...
def create_model_worker_app(log_level: str = "INFO", **kwargs) -> FastAPI:
    ...
    logger.info(f"启动worker进程:{kwargs}")
...
def run_openai_api(log_level: str = "INFO", started_event: mp.Event = None):
    ...
    logger.info("开始启动OpenAI API接口服务....")

    controller_addr = fschat_controller_address()
    logger.info(f"对应的controller地址路径为:{controller_addr}")
...
async def start_main_server():
    ...
    # 启动模型管理服务controller的进程 + OpenAI 接口规范的服务进程
    if args.openai_api:
        # 多进程的写法---启动一个字进程
        process = Process(
    ...
    if args.model_worker:
        for model_name in args.model_name:
            ...
            if not config.get("online_api"):
                logger.info(f"开始启动本地模型worker对应的进程:{model_name} - {config}")
                e = manager.Event()
    ...
    # 启动第三方的模型 -- 并且是通过worker class代码访问的模型服务
    if args.api_worker:
        for model_name in args.model_name:
            config = get_model_worker_config(model_name)
            if (config.get("online_api")
                ...
                logger.info(f"开始启动API远程接口模型worker对应的进程:{model_name} - {config}")
                e = manager.Event()
                model_worker_started.append(e)
                process = Process(
                    target=run_model_worker,
