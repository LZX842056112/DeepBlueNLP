...
def __train(run_args):
    ...
    trainer.train(
        train_path=run_args.train_path,  # 训练数据所在文件路径
        valid_path=run_args.valid_path,  # 验证数据所在文件路径
        types_path=run_args.types_path,  # 元数据所在文件路径
        input_reader_cls=input_reader.JsonInputReader  # 数据加载class
    )
