...
with open(os.path.join(root_path, 'label_data', 'city_list.txt'), 'r', encoding='utf8') as fr:
...
def get_weather_plant(weather, ret_dict):
    plant = db.findOtherEntities(weather, "适合种植")  # 基于主体和关系，查询客体
    ...
    for i in selected_index:
        ...
        if selected_plant[-1] == "科":
            concrete_plant_list = db.findOtherEntities2(selected_plant, "科")  # 获取属于当前科的所有植物列表
...
def get_xian_plant(address, ret_dict):
    upper_address = get_shi_address(address)  # address对应的上一级城市

    if upper_address in city_list:
        ret_dict = get_shi_plant(upper_address, ret_dict)  # 或者这个市适宜种植的农作物
...
pattern = [[r"适合种什么", r"种什么好", r"种[\u4e00-\u9fa5]+什么"],
...
def inner_question_answering(question):
    cut_statement = thu_lac.cut(question, text=False)  # 分词和词性标注
    print(cut_statement)
    address_name = []
    weather_name = []
    question_name = ""
    ...
    pos = -1  # 具体匹配问题的在question中的起始下标
    q_type = -1  # 保存问题类别 ---> 意图识别(可以考虑用模型来判断当前所属问题具体是那种问题)
    for i in range(len(pattern)):
        for x in pattern[i]:
            index = re.search(x, question)
            if index:
                pos = index.span()[0]  # 匹配的开始位置
                q_type = i  # 匹配那种问题
                break
        if pos != -1:
            break
    ...
    print(q_type, pos)
    # 匹配问题 xxx地方适合种什么  ---> 模板槽位匹配的检索方式 slot槽
    if q_type == 0:
        index = 0
        # 在 pos 之前的位置找地名实体信息
...
def question_answering(request):  # index页面需要一开始就加载的内容写在这里
    ...
    context = {'ctx': ''}
    if request.GET:
        question = request.GET['question']  # 请求过来的问题字符串
        ret_dict = inner_question_answering(question)  # 调用后端逻辑，针对问题返回结果
        if len(ret_dict) != 0 and ret_dict != 0:
