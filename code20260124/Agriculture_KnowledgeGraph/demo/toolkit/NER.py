...
def get_NE(text):
	...
	while i < length:
		p1 = TagList[i][0] # 当前单词
		t1 = TagList[i][1] # 当前单词对应词性
		p2 = TagList[i+1][0] # 下一个单词
		t2 = TagList[i+1][1] # 下一个单词对应的词性
		p12 = p1 + TagList[i+1][0] # 当前单词和下一个单词合并到一起的一个短语
		
		# 不但需要txt中有实体，还需要判断数据库中有没有
		flag = db.matchHudongItembyTitle(p12) 
		if p12 in label and flag is not None and preok(t1) and nowok(t2):  # 组合2个词如果得到实体
			answerList.append([p12,label[p12]])
			i += 2
			continue
		
		flag = db.matchHudongItembyTitle(p1)
		if p1 in label and flag is not None and nowok(t1):	 # 当前词如果是实体
