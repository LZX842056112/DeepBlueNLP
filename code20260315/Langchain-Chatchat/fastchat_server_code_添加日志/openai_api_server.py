...
async def get_gen_params(
    ...
    *,
    ...
    logger.info(f"当前模型对应的数据模版名称为 {model_name} {worker_addr} {conv}")
    conv = Conversation(
        name=conv["name"],
...
async def create_chat_completion(request: ChatCompletionRequest):
    ...
    print(f"OpenAI API接口内部接收到的参数为: \n{request}\n\n")
    logger.info(f"OpenAI API接口内部接收到的参数为: \n{request}\n\n")
    ...
    print(f"OpenAI API接口内部转换后的参数为: \n{gen_params}\n\n")
    logger.info(f"OpenAI API接口内部转换后的参数为: \n{gen_params}\n\n")
